#include "lanes_costmap_plugin/lane_layer.hpp"
...
LaneLayer::LaneLayer() : last_min_x_(-std::numeric_limits<float>::max()), last_min_y_(-std::numeric_limits<float>::max()), last_max_x_(std::numeric_limits<float>::max()), last_max_y_(std::numeric_limits<float>::max())
{
    cv::Mat img; // ensure openCV works
}
...
void LaneLayer::onInitialize()
{
    ...
    // create subscribers
    pointcloudSub = node->create_subscription<PointCloud2>(pointcloud_topic, 10, std::bind(&LaneLayer::pointcloudCb, this, _1));
    imageSub = node->create_subscription<Image>(image_topic, 10, std::bind(&LaneLayer::imageCb, this, _1));
...
void LaneLayer::updateCosts(nav2_costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
    if (!enabled_) return;
...
PLUGINLIB_EXPORT_CLASS(lanes_costmap_plugin::LaneLayer, nav2_costmap_2d::Layer)
